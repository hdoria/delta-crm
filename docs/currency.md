...
- **`CURRENCIES` is twelve currencies and that is all this CRM supports** — USD, EUR,
  JPY, GBP, CNY, AUD, CAD, CHF, HKD, SGD, ZAR, BRL, in array order. `isCurrencyCode` is
  the single gate for the picker, the feed filter and the stored setting. A refresh
  **prunes** `FETCHED` rows outside the list and leaves `MANUAL` alone.
- **The `name` on each entry is the English fallback, not what the UI shows.** The
  picker reads `messages/*/currency.json`, keyed by code, so the list of codes and the
  words a reader sees stay separate. A new currency needs both.
