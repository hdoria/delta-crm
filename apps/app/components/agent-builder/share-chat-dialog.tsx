...
export function ShareChatDialog({
	...
	const t = useTranslations("agentBuilder.shareChat");
	const trpc = useTRPC();
	...
	const revoke = useMutation(
		trpc.conversations.revokeShare.mutationOptions({
			onSuccess: async (_result, input) => {
				...
				await invalidate();
				toast.success(t("linkRevokedToast"));
			},
			onError: (error) => toast.error(error.message),
		}),
	);
			...
			>
				<DialogHeader className="relative gap-1 border-b p-5 pr-14">
					...
					<DialogClose asChild>
						...
						>
							<Icon icon={Close} />
							<span className="sr-only">{t("closeAria")}</span>
						</Button>
					</DialogClose>
				</DialogHeader>
					...
					{status.isPending ? (
						...
						<>
							...
							<ShareChoice
								...
								label={t("anyoneWithLink", { appName: BRAND.appName })}
								detail={t("readOnly")}
								disabled={createAction.pending || revokeAction.pending}
								onSelect={() => {
								...
								<div className="flex h-9 items-center gap-3 rounded-md border bg-background py-[3px] pr-[3px] pl-4">
									...
									>
										...
										>
											...
											{shareToken ? t("copyLink") : t("replaceLink")}
										</AsyncButtonContent>
									</Button>
								</div>
							) : null}

							<p className="text-muted-foreground text-xs">{t("shareNote")}</p>
						</>
					)}
				</div>
