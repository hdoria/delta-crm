...
function columns(
	t: ReturnType<typeof useTranslations<"settings.sso">>,
	...
	return [
				...
				<span className="flex min-w-0 flex-col">
					...
					<span className="truncate text-muted-foreground text-xs">
						...
						{row.clientIdLastFour
							? ` · ${t("clientHint", { last4: row.clientIdLastFour })}`
							: ""}
					</span>
				</span>
			),
		...
		{
			id: "callbackURL",
			header: t("redirectUriColumn"),
			width: "w-[20%]",
			hideBelow: "lg",
			cell: (row) => (
				<span className="flex min-w-0 items-center gap-1 text-muted-foreground">
					<span className="truncate">{row.callbackURL}</span>
					<CopyValue value={row.callbackURL} label={t("redirectUriLabel")} />
				</span>
			),
		},
		{
				...
				canConfigure ? (
					<AlertDialog>
						...
						<AlertDialogContent>
							<AlertDialogHeader>
								<AlertDialogTitle>
									{t("removeConfirmTitle", { name: row.name })}
								</AlertDialogTitle>
								<AlertDialogDescription>
